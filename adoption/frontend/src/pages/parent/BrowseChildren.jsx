...
import { CHILD_STATUS } from "../../utils/constants";
...
function BrowseChildren() {
  ...
  // Get children that are in active applications
  const childrenInApplication = applications
    .filter(app => !["REJECTED"].includes(app.status))
    .map(app => app.childId);
  
  // Filter available children (not in application, not adopted)
  const availableChildren = childrenData.filter(child => 
    child.status === CHILD_STATUS.AVAILABLE && 
    !childrenInApplication.includes(child.id)
  );
